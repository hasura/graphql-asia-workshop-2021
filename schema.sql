...
CREATE TABLE "order" (
    ...
    order_total numeric
    -- TODO: Hasura computed field to calculate order total based on SQL function to sum all (order.product.price * order.product.quantity)
);
...
CREATE TRIGGER sum_order BEFORE INSERT OR UPDATE ON public.order
    FOR EACH ROW EXECUTE PROCEDURE public.gen_order_total();CREATE TRIGGER set_order_product_updated_at
